...
s = parse_stats(first_block(path))
...
# context-switch window cache/TLB (only if access lines exist)
if ca.get('access') and cb.get('access'):
    ...
    for k, label in {'l1d': 'L1-D', 'l2': 'L2', 'dtlb': 'dTLB', 'itlb': 'iTLB'}.items():
...
# whole workload from gem5 stats.txt
l1_acc = l1_mis = 0
...
_, dra = find(s, r'mmu\.dtb\.readAccesses'); _, dwa = find(s, r'mmu\.dtb\.writeAccesses')
_, drm = find(s, r'mmu\.dtb\.readMisses');   _, dwm = find(s, r'mmu\.dtb\.writeMisses')
if None not in (dra, dwa, drm, dwm):
    acc, mis = dra + dwa, drm + dwm
    print(f"{'dTLB':6}{acc:14,.0f}{mis:12,.0f}{(100*mis/acc if acc else 0):13.3f}{(1000*mis/insts if insts else 0):10.3f}")
_, ia = find(s, r'mmu\.itb\.instAccesses'); _, im = find(s, r'mmu\.itb\.instMisses')
if None not in (ia, im):
