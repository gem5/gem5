# implementing section VI B scenario 1: 50/50 partition of the 4-way L3 between the two active domains.
# domain 0 has ways {0,1}; domain 1 has ways {2,3}

# system spec as per Table I simulated system specifications:
# 32KB, 8-way L1s
...
l3_size_bytes = 16 * 1024
num_ways = int(system.l3cache.assoc)
num_sets = l3_size_bytes // (int(system.cache_line_size) * num_ways)

lower_mask = 0x3  # ways 0,1
upper_mask = 0xC  # ways 2,3

wg = objects.WayGuardTable(
    num_sets=num_sets,
    num_ways=num_ways,
    initial_masks=[f"0:0x{lower_mask:x}", f"1:0x{upper_mask:x}"],
)
system.l3cache.tags.way_guard_table = wg
