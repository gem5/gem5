...
def main():
    parser = argparse.ArgumentParser(
        description="Wrapper for gem5 SLICC compiler"
    )
    parser.add_argument("--protocol", help="The protocol .slicc file to parse")
    parser.add_argument(
        "--includes", help="Comma separated list of shared .slicc files"
    )
    parser.add_argument(
        "--output_dir", "-C", help="Output directory for generated C++ files"
    )
    parser.add_argument(
        "--cc_includes",
        help="Comma separated list of C++ headers to include in generated files",
    )
    parser.add_argument(
        "positional_includes",
        nargs="*",
        help="Positional .slicc include files",
    )
    ...
    if not args.protocol and not includes:
        print(
            "Error: Either --protocol or at least one include file must be provided.",
            file=sys.stderr,
        )
        sys.exit(1)

    slicc = SLICC(args.protocol, includes, protocol_base, verbose=True)
