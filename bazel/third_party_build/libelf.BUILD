...
genrule(
    ...
    cmd = "m4 -DSRCDIR=$$(dirname $(location libelf_convert.m4)) $(location libelf_convert.m4) > $@",
)
...
genrule(
    ...
    outs = ["libelf_fsize.c"],
    cmd = "m4 -DSRCDIR=$$(dirname $(location libelf_fsize.m4)) $(location libelf_fsize.m4) > $@",
)
...
genrule(
    ...
    outs = ["libelf_msize.c"],
    cmd = "m4 -DSRCDIR=$$(dirname $(location libelf_msize.m4)) $(location libelf_msize.m4) > $@",
)
...
genrule(
    ...
    cmd = "chmod +x $(location native-elf-format) && $(location native-elf-format) > $@",
)
