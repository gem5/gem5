...
class ContextBasedPrefetcher:
    def __init__(self, prefetch_window, reward_function):
        ...
        self.previousAccesses = deque(maxlen=32)  # Store last 4 addresses
        self.previousPCs = deque(maxlen=4)       # Store last 4 PCs
        self.prefetchQueue = deque()
        self.reward_threshold = 1200
        self.rewards = []

    def hash(self, addr, pc):
        # Combine the last 4 addresses and PCs into a single hash value
        hash_value = 0
        prime = 2654435761  # A large prime for mixing

        # Mix addresses
        for a in self.previousAccesses:
            hash_value = (hash_value * prime) ^ ((a >> 2) & 0x3FF)  # Use lower bits of address

        # Mix PCs
        for p in self.previousPCs:
            hash_value = (hash_value * prime) ^ (p & 0x3FF)  # Use lower bits of PC

        # Mix the current address and PC
        hash_value = (hash_value * prime) ^ ((addr >> 2) & 0x3FF)
        hash_value = (hash_value * prime) ^ (pc & 0x3FF)

        # Keep the result within a 10-bit range
        hash_value &= 0x3FF
        return hash_value
    ...
    def predict(self, pc, addr):
        # Update the sliding window of addresses and PCs
        self.previousAccesses.append(addr)
        self.previousPCs.append(pc)

        # Compute the hash for the current state
        key = self.hash(addr, pc)

        # Add the current address to the state
        for a in self.previousAccesses:
            prev_key = self.hash(a, pc)
            self.addToState(prev_key, addr)

        # Get the best address and reward
        ...
        # Update scores and previous accesses
