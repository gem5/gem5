...
runBenchmark()
{
    ...
    for (std::size_t output_idx = 0; output_idx < perf::kMacOutputs;
        ...
        for (std::size_t tap = 0; tap < perf::kMacTaps; ++tap) {
            ...
            accumulator =
                perf::macCustom(accumulator, gMacLhs[lhs_idx], gMacRhs[rhs_idx]);
        }

        const std::uint32_t clamped =
            perf::clampCustom(accumulator, perf::kMacClampMax);
        checksum = perf::mix32(
            checksum,
...
_start()
{
    ...
    perf::endKernelStats();
    perf::printBenchmarkResult("mac_clamp_custom", checksum);
    perf::syscallExit(0);
}
